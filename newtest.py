...
def main():
    ...
    for _, row in df.iterrows():
        try:
            ...
            if not output_df.empty and name in output_df["NAME"].values:
                # Update existing row
                output_df.loc[output_df["NAME"] == name, today_col] = push_status
                output_df.loc[output_df["NAME"] == name, "Pushed_Today"] = pushed_today
            else:
                # Add new row
                new_row = {
                    "NAME": name,
