...
def main():
    ...
    for new_row in updated_data:
        name = new_row["NAME"]
        git_url = new_row["GITLINK"]

        if not existing_df.empty and ((existing_df["NAME"] == name) & (existing_df["GITLINK"] == git_url)).any():
            existing_df.loc[(existing_df["NAME"] == name) & (existing_df["GITLINK"] == git_url), today_col] = new_row[today_col]
            existing_df.loc[(existing_df["NAME"] == name) & (existing_df["GITLINK"] == git_url), "Pushed_Today"] = new_row["Pushed_Today"]
        else:
